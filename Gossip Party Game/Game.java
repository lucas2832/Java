...
public class Game {
    ...
    private static ArrayList <Npc> oldNpc = new ArrayList<Npc>();
    ...
    private void createItems() {
        Item Vodka = new Item ("Vodka",36,0,false);
        Item Caipirinha = new Item ("Caipirinha",28,0,false);
        Item XequeMate = new Item ("Xeque-Mate",23,0,false);
    ...
    private boolean processCommand(Command command) throws InterruptedException {
        ...
        if (commandWord.equals("help")) {
            ...
            if(Game.currentNpc.getName().equals("Barman")) {
                ...
                System.out.println("* você precisa ir ate o barman para pedir algo *");
            }
        ...
        if (Player.deuPT()) {
            Janela pt = new Janela(Janela.PTLabel);
            wantToQuit = Player.deuPT();
        }
        if (Player.esfomeousse())
        {
            Janela pt = new Janela(Janela.fomeLabel);
            wantToQuit = Player.deuPT();
        }
        return wantToQuit;
    }
    ...
    public void drinkLeftHand() {
        if (!Player.leftHand.isComida() && Player.getLeftHand() != null) {
            Player.drunkUpLeftHand();
            System.out.println("seu nivel de alcolizado agora é: "+Player.getDrunkValue());
        } else {
            System.out.println("* você precisa ter um drink nessa mão para beber *");
        }
    }

    public void drinkRightHand() {
        if (!Player.rightHand.isComida() && Player.getRightHand() != null) {
    ...
    public void eatRightHand() {
        if (Player.rightHand.isComida()) {
            ...
            System.out.println("seu nivel de fome agora é: "+ Player.getHungerValue());
        } else {
    ...
    /*public void back() {
        if (oldNpc.size() > 0) {
            currentNpc = oldNpc.get(oldNpc.size() - 1);
            allNpcs.remove(oldNpc.size() - 1);
        } else {
